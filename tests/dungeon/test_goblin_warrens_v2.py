"""Smoke tests for goblin_warrens_v2 — content authoring conforms to the
v2 schema and the engine generates playable floor states on top of it.

Mirrors test_cartographers_folly_v2 in shape; the assertions specific
to the Goblin Warrens are the boss IDs and the legendary item.
"""
...
def test_legendary_reward_is_goblin_kings_crown():
    legendary = _dungeon().get("legendary_reward") or {}
    assert legendary.get("item_id") == "amulet_of_vitality"
    assert legendary.get("name") == "The Goblin King's Crown"
...
# ---------------------------------------------------------------------------
# Bosses match the v1 roster (no behavior change for combat).
# ---------------------------------------------------------------------------


def test_floor_bosses_are_chief_troll_king():
    floors = _dungeon()["floors"]
    assert floors[0]["boss"]["id"] == "goblin_chief"
    assert floors[1]["boss"]["id"] == "troll_guardian"
    assert floors[2]["boss"]["id"] == "goblin_king"
...
def test_branched_layout_produces_junctions(seed: int):
    """The Goblin Warrens YAML asks for branches on every floor; the
